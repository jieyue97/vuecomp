const arr = [
  {
    ...
    tableData: [
      ...
      {
        name: 'isTableAuto',
        desc: '是否根据屏幕来自动计算表格初始页显示数据条数，若设置，在方法@getReallySize="getReallySize有返回初始值"',
        paramsType: 'Boolean',
        value: 'false',
        defaultValue: 'false',
      },
      {
        name: 'bottomOffset',
        desc: '表格距离底部距离，影响用来计算table最大高度',
        paramsType: 'Number',
        value: '0',
        defaultValue: '0',
      },
      {
        name: 'tablePaddingOffset',
        desc: '计算表格初始页显示数据条数时，因为渲染前不能计算表格的底部距离，所以默认距离是16，（--提醒：bottomOffset设置了表格底部距离，所以，如果首次计算有滚动条了，即实际dom height超过了maxheight，应该设置tablePaddingOffset+分页高度（默认42）>=bottomOffset，才不会出现滚动条）',
        paramsType: 'Number',
        value: '16',
        defaultValue: '16',
      },
  ...
  },
  {
    ...
    tableData: [
      {
        name: 'headerHtml',
        desc: '放在列数据的对象里面，主要用于多表头，展示多表头情况下的需要其他操作处理数据的方法，',
        params: '--',
        method: 'headerHtml:function(row,item){return row[item.prop]}',
      },
      {
        name: 'pagination',
        desc: '分页参数抛出',
      ...
      {
        name: 'getReallySize()',
        desc: '@getReallySize="getReallySize"',
        params: '',
        method: '渲染时计算表格适应行数',
      },
    ],
